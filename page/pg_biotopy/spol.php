                ...
                if ($dbtab) {
                    ...
                    <table border=1>
                        <tr>
                                ...
                                if (pg_field_type($results, $gt) != "geometry") {
                                    if (pg_field_name($results, $gt) == "gid" || pg_field_name($results, $gt) == "name" || pg_field_name($results, $gt) == "editable_svd" || pg_field_name($results, $gt) == "hyp_max" || pg_field_name($results, $gt) == "editable_title") {
                                        ...
                                        if (pg_field_name($results, $gt) == "group") {
                                            ...
                                            echo "body (dnes)";
                                        } elseif (pg_field_name($results, $gt) == "a_svds") {
                                            echo "SVDs";
                                        } elseif (pg_field_name($results, $gt) == "a_spol_hod") {
                                            echo "aktuální společenská hodnota";
                                        } elseif (pg_field_name($results, $gt) == "p_body") {
                                            echo "body (potenciální)";
                                        } elseif (pg_field_name($results, $gt) == "p_svds") {
                                            echo "SVDs";
                                        } elseif (pg_field_name($results, $gt) == "p_spol_hod") {
                                            echo "společenský potenciál";
                                        } else {
                            ...
                            <form method="POST" action="../../action/pgItem_update_bio.php" enctype="multipart/form-data"  data-ajax="false">
                                ...
                                unset($gid);
                                for ($gt = 0; $gt < pg_num_fields($results); $gt++) {
                                    if (pg_field_name($results, $gt) == "group" || pg_field_name($results, $gt) == "title" || pg_field_name($results, $gt) == "a_body" || pg_field_name($results, $gt) == "a_svds" || pg_field_name($results, $gt) == "p_svds" || pg_field_name($results, $gt) == "a_spol_hod" || pg_field_name($results, $gt) == "p_body" || pg_field_name($results, $gt) == "p_spol_hod") {
                                        if (pg_field_name($results, $gt) == "title") {
                                        ...
                                        } elseif (pg_field_name($results, $gt) == "a_svds" || pg_field_name($results, $gt) == "p_svds" || pg_field_name($results, $gt) == "a_spol_hod" || pg_field_name($results, $gt) == "p_spol_hod") {
                                            if (pg_fetch_result($results, $lt, "editable_svd") == "0") {
                                                ...
                                                if (pg_field_name($results, $gt) == "a_spol_hod" || pg_field_name($results, $gt) == "p_spol_hod") {
                                                    echo('<td>');
                                                    echo('<input type="text" value="' . pg_result($results, $lt, $gt) . '" name="' . pg_field_name($results, $gt) . '" readonly="readonly"  style="background-color:#DDDDDD;">');
                                                } else {
                                                    echo('<td>');
                                            ...
                                            if (pg_fetch_result($results, $lt, "group") == "sidelni") {
                                                echo('<td>');
                                            ...
                                            } elseif (pg_fetch_result($results, $lt, "group") == "rekreacni") {
                                                echo('<td>');
                                            ...
                                            } elseif (pg_fetch_result($results, $lt, "group") == "kulturni") {
                                                echo('<td>');
                    ...
                    <form >
                        <select name="group" id="tabMenuContentGroup">
                            <option value="sidelni">Sídelní</option>
                            <option value="rekreacni">Rekreační</option>
                            <option value="kulturni">Kulturní a edukační</option>
                            <option value="enviromentalni">Enviromentální</option>
                        </select>
                    ...
                    <tr>
                        ...
                        for ($gt = 0; $gt < pg_num_fields($results); $gt++) {
                            ...
                            if (pg_field_name($results, $gt) == "sum_akt") {
                                ...
                                echo "potenciální společenská hodnota";
