...
//prihlasim uzivatele
    ...
    function requestJSON($json) {
        ...
        if ($this->request->userHash) {
            $this->user = new LGUser();
            $this->user->hash($this->request->userHash);
            if(!$this->user->isUser()){
                $this->user=null;
            }
        }elseif($this->request->request == "user.login"){
            $this->user = new LGUser();
        }
    }
    ...
    function getCapabilities() {
        /**
         * ted potrebuji:
         * 1/ plĂˇny, na kterĂ© mĂˇm prĂˇvo edit/view
         * 2/ vrstvy danĂ˝ch plĂˇnĹŻ (dle pĹ™ihlĂˇĹˇenĂ­ dĂˇm private/public vrstvy)
         * 3/ formulĂˇĹ™e danĂ© vrstvy (dĂˇm jen pro prĂˇvo edit)
         * 
         * 4/ uloĹľit data z form
         * 5/ dĂˇt form data
         * 
...
//  $this->user = new LGUser();
    ...
    /**
     * @todo not work, problem with privileges
     * @return type 
     */
    function planList() {
        $temp = array();
